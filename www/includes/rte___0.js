...
function encontra_destino(){
	...
	if (temp.lastIndexOf("=") == -1) onde = "conteudo";
	else{
		temp = temp.split("=");
		onde = temp[1];
	}
...
function _initEditor() {
	...
	if (self.opener.document.getElementById(DESTINO).value != ""){
		idEditbox.document.body.innerHTML = self.opener.document.getElementById(DESTINO).value;
	}
...
function _drawModeSelect() {
	var sz 	= "<TABLE CELLSPACING=0 CELLPADDING=0 ID=idMode width='97%'>"
			...
			+ "<TD><input type='button' value='OK' onclick='if (!cbMode.checked) self.opener.document.all[\"" + DESTINO + "\"].value = idEditbox.document.body.innerHTML; else self.opener.document.all[\"" + DESTINO + "\"].value = idEditbox.document.body.innerText; self.close();'></td>"
			+ "</TR></TABLE>";
	document.write(sz);
