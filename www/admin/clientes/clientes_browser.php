...
if ($modo == "salvar"){
	...
	while (list($chave, $valor) = each ($_POST)) {
		if ($chave != "Nova"){
			$query = "UPDATE clientes SET ordem='" . $valor ."' WHERE (cd='" . $chave . "') LIMIT 1";
			$result = mysql_query($query) or die("Erro ao remover registros do Banco de dados: " . mysql_error());	
		}
	}
}

$result = mysql_query("SELECT * FROM clientes order by ordem" . $query_limit) or die("Erro ao acessar registros no Banco de dados: " . mysql_error());	
...
<html>
	<head>
		...
		<script language="JavaScript" type="text/javascript">
			...
			function pg_inicial(){
				document.all["frm_edita"].src = 'pginicial_form.php';
			}
		</script>
		...
		<table class="tabela" width="100%">
			<tr>
				<td width="50%" valign="top">
					<table class="tabela" width="100%">
					<form name="dados" action="clientes_browser.php?modo=salvar" method="post">
						<tr>
