...
if ($modo == "apagar"){
	...
	while (list($chave, $valor) = each ($_POST)) {
		if (($chave != "allbox") && ($chave != "1") && ($valor == "on")){
			$query = "DELETE FROM internautas WHERE (cd='" . $chave . "') LIMIT 1";
...
if ($filtro != "") $result = mysql_query("SELECT * FROM internautas" . $filtro_sql . "order by nome" . $query_limit) or die("Erro ao acessar registros no Banco de dados: " . mysql_error());	
else $result = mysql_query("SELECT * FROM internautas order by nome" . $query_limit) or die("Erro ao acessar registros no Banco de dados: " . mysql_error());	

$query = "SELECT COUNT(*) FROM internautas";
if ($filtro != ""){
	$query .= $filtro_sql;
}
...
<html>
	<head>
		<title>Usuários da Newsletter</title>
		<style type="text/css">
			<!--
		...
		<script language="JavaScript" type="text/javascript">
			...
			function novo(){
				window.open('inter_form.php?modo=add', 'Usuario', 'width=410,height=110,status=no,resizable=no,top=20,left=100,dependent=yes,alwaysRaised=yes');
			}
			function apagar(){
				if (window.showModalDialog('../includes/confirmacao.html',['Confirme!','Deseja apagar os usuários selecionados?','Sim','Não'],'dialogWidth:320px;dialogHeight:100px;status:no;') == "1"){
					document.all["seleciona_users"].action = "inter_browser.php?modo=apagar";
					document.all["seleciona_users"].submit();
				}
			}
			function edita_usuario(codigo){
				window.open('inter_form.php?modo=update&cd=' + codigo, 'Usuario', 'width=410,height=110,status=no,resizable=no,top=80,left=100,dependent=yes,alwaysRaised=yes');
			}
	...
	<body leftmargin="0" bottommargin="0" rightmargin="0" topmargin="0" marginheight="0" marginwidth="0" onMouseDown="escondemenu();">
		...
		<FORM name="paginacao" action="inter_browser.php" method="post">
			<tr>
				...
				<td class="label" width="70%">&nbsp;Busca:&nbsp;&nbsp;<input type="text" name="filtro" class="textfield" style="width: 85%" value="<?=$filtro?>">&nbsp;&nbsp;<input type="submit" value="OK" class="botao" style="width: 5%"></td>
		...
		<table class="tabela" width="100%">
			...
			<tr>
				...
				<td class="textfield" align="center"><b>E-mail</b></td>
			</tr>
			<?php for($i = 0; $i < $qtd; $i++){
		...
		<table class="tabela" width="100%">
		<FORM name="paginacao2" action="inter_browser.php" method="post">
			<tr>
				<td class="label" width="10%" align="center"><input type="button" class="botao" onClick="javascript: novo();" value="Novo" style="width: 100%"></td>
				<td class="label" width="10%" align="center"><input type="button" class="botao" onClick="javascript: apagar();" value="Apagar" style="width: 100%"></td>				
					<input type="hidden" name="pagina" value="<?=$pagina?>">
				<td class="label" align="center" width="30%">Internautas/Pagina:&nbsp;<input type="text" class="textfield" name="qtd" value="<?=$qtd?>" onBlur="mudapagina2('');" size="3" maxlength="3" alt="Digite a quantidade de imagens por página a serem mostradas."></td>
				<td class="label" align="center" width="30%"><?=$eof?>&nbsp;Internautas cadastrados.</td>
				<td class="label" width="5%"><input type="button" style="width: 100%" class="botao" name="back" value="<" onclick="mudapagina('back');" <?php if($pagina == 0 || $pagina == 1) echo("disabled");?>></td>
				<td class="label" width="5%"><input type="button" class="botao" style="width: 100%" name="next" value=">" onclick="mudapagina('next');" <?php if($limite + $qtd >= $eof) echo("disabled");?>></td>
			</tr>
		</form>
