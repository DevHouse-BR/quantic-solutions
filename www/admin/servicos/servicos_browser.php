...
if ($modo == "salvar"){
	...
	while (list($chave, $valor) = each ($_POST)) {
		if ($chave != "Nova"){
			$query = "UPDATE servicos SET ordem='" . $valor ."' WHERE (cd='" . $chave . "') LIMIT 1";
			$result = mysql_query($query) or die("Erro ao remover registros do Banco de dados: " . mysql_error());	
		}
	}
}

$result = mysql_query("SELECT * FROM servicos order by ordem" . $query_limit) or die("Erro ao acessar registros no Banco de dados: " . mysql_error());	
...
<html>
	<head>
		...
		<script language="JavaScript" type="text/javascript">
			...
			function introducao(){
				document.all["frm_edita"].src = 'intro_form.php';
			}
		</script>
		...
		<table class="tabela" width="100%">
			<tr>
				<td width="50%" valign="top">
					<table class="tabela" width="100%">
					<form name="dados" action="servicos_browser.php?modo=salvar" method="post">
						<tr>
							...
							<td class="label" width="10%" align="center"><input type="button" class="botao" onClick="javascript: introducao();" value="Introdução" style="width: 100%"></td>
						</tr>
					</table>
					<table class="tabela" width="100%">
						<tr>
				...
				<td width="50%" valign="top">
					<iframe name="frm_edita" width="100%" scrolling="no" height="250"></iframe>
				</td>
			</tr>
		</table>
	</body>
